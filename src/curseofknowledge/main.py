...
class results(InteractiveScene, Scene2D):
    def construct(self):
        ...
        # y_range를 값 범위(min~max)에 맞추면 축의 0이 평면 밖으로 나간다.
        # manimgl은 축을 그릴 때 n2p(0)이 원점에 오도록 밀어버리기 때문에
        # x축이 평면에서 떨어져 나가 화면이 깨진다. 그래서 y축은 항상 0에서
        # 시작하는 고정 범위로 두고, 값 구간만 위쪽 띠(BAND)에 넓게 펼친 뒤
        # 잘라낸 0~데이터 구간은 물결 표시로 알린다.
        Y_MAX = 5.0  # plane y_range 상단
        BAND = (1.0, 4.6)  # 세 선이 실제로 그려지는 구간 (plane y 좌표)
        Y_BREAK = 0.45  # 물결 표시를 놓을 높이

        # 패널마다 세 값을 조금 여유를 두고 감싸는 가격 구간
        price_bands = []
        for i in range(8):
            lo = min(x_predictions_list[i], equil[i], actual[i])
            hi = max(x_predictions_list[i], equil[i], actual[i])
            pad = max((hi - lo) * 0.2, 0.05)
            price_bands.append((lo - pad, hi + pad))

        def p2y(i, price):
            """가격 -> 해당 패널의 plane y 좌표."""
            lo, hi = price_bands[i]
            return BAND[0] + (price - lo) / (hi - lo) * (BAND[1] - BAND[0])
        ...
        def axis_break(nump, y=Y_BREAK, width=0.26, amp=0.035, gap=0.13):
            """축이 잘렸다는 물결(~~) 표시. 두 줄 사이는 마스크로 축선을 지운다."""
            center = nump.c2p(0, y)
            mask = (
                Rectangle(width=width, height=gap)
                .set_fill(BLACK, 1)
                .set_stroke(width=0)
                .move_to(center)
            )
            waves = VGroup(
                *[
                    FunctionGraph(
                        lambda t: amp * np.sin(TAU * t / width),
                        x_range=[-width / 2, width / 2, width / 40],
                        color=GREY_B,
                        stroke_width=2,
                    ).move_to(center + UP * dy)
                    for dy in (-gap / 2, gap / 2)
                ]
            )
            return VGroup(mask, waves)

        breaks = VGroup(*[axis_break(nump) for nump in numps])
        ...
        self.playw(FadeIn(numps), FadeIn(breaks), FadeIn(atoh))
        ...
        def get_line(i, price, color=GREEN_B, dashed=True):
            ...
            return cls(nump.c2p(0, y), nump.c2p(5, y), color=color)

        # 값이 거의 붙어 있는 패널(A의 3.47/3.49)은 라벨끼리 겹치므로,
        # 선은 제 위치에 두고 라벨만 최소 간격을 확보해 위아래로 벌린다.
        LABEL_GAP = 0.62  # plane y 단위

        def spread(ys, gap=LABEL_GAP):
            out = list(ys)
            order = sorted(range(len(ys)), key=lambda k: ys[k])
            for a, b in zip(order, order[1:]):
                out[b] = max(out[b], out[a] + gap)
            # 위로만 밀렸으니 전체를 다시 내려 원래 중심을 유지한다
            shift = (sum(out) - sum(ys)) / len(ys)
            return [y - shift for y in out]

        label_ys = [
            spread([p2y(i, vals[i]) for vals in (x_predictions_list, equil, actual)])
            for i in range(8)
        ]

        def get_label(i, k, price, color):
            return (
                Text(f"${price:.2f}", font_size=20)
                .next_to(numps[i].c2p(5, label_ys[i][k]), RIGHT, buff=0.1)
                .set_color(color)
            )
        ...
        texts = VGroup(
            *[get_label(i, 0, x_predictions_list[i], RED) for i in range(8)]
        )
        legends = VGroup()
        leg1 = VGroup(
        ...
        texts_equil = VGroup(*[get_label(i, 1, equil[i], BLUE) for i in range(8)])
        self.playw(*[Create(line) for line in lines_equil], FadeIn(texts_equil, shift=RIGHT * 0.1), FadeIn(legends[1]), *[Create(line) for line in lines], FadeIn(texts, shift=RIGHT * 0.1), FadeIn(legends[0]))

        lines_actual = VGroup(
            *[get_line(i, actual[i], color=GREEN_B) for i in range(8)]
        )
        texts_actual = VGroup(*[get_label(i, 2, actual[i], GREEN_B) for i in range(8)])
        self.playw(*[Create(line) for line in lines_actual], FadeIn(texts_actual, shift=RIGHT * 0.1), FadeIn(legends[2]))

        ## camera1
        self.cf.save_state()
        self.playw(self.cf.animate.reorient(0, 0, 0, (np.float32(-2.14), np.float32(1.18), np.float32(0.0)), 3.74))

        self.embed()
        ## camera2

        self.playw(self.cf.animate.reorient(0, 0, 0, (np.float32(3.04), np.float32(1.36), np.float32(0.0)), 3.60))
